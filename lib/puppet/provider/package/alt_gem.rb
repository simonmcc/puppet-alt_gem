...
# Ruby gems support.
Puppet::Type.type(:package).provide :alt_gem, :parent => Puppet::Provider::Package do
  desc "Alternative Ruby Gem support.  
    Support maintaining gems installed somewhere outside the std path.
    If a URL is passed via `source`, then that URL is used as the
  ...
  default_gemcmd = '/opt/ruby-enterprise/bin/gem'
  commands :gem => '/opt/ruby-enterprise/bin/gem'
  
  def self.gemlist(options)
    gem_list_command = [command(:gem), "list"]
  ...
  def uninstall
    command = [command(:gem), "uninstall", "-x", "-a", resource[:name]]
    output = execute(command)
  end
